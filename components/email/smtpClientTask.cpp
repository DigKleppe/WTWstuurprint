...
void smtpClientTask(void *pvParameters) {
	char *buf = NULL;
	...
	if (err == ESP_OK) {
		...
		CHECKRESULT_SET_ERROR(ret, 200, 299);
		if (err == ESP_OK) {
		...
		CHECK_RESULT_SET_ERR(ret, 200, 299, exit);
		ESP_LOGI(TAG, "Writing EHLO to server...");
	...
	if (err == ESP_OK) {
		...
		ret = write_ssl_and_get_response(&ssl, (unsigned char *)buf, len);
		CHECKRESULT_SET_ERROR(ret, 200, 399);
	}
	...
	if (err == ESP_OK) {
		...
		ret = write_ssl_and_get_response(&ssl, (unsigned char *)buf, len);
		CHECKRESULT_SET_ERROR(ret, 300, 399);
	}
